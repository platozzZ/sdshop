...
export function LaiKeTui_axios(me) {
	...
	uni.request({
		...
		method: 'POST',
		success: (res) => {
			console.log('LaiKeTui_axios:',res.data)
			uni.hideLoading();
...
export function LaiKeTui_buy_handle(me) {
	// console.log('LaiKeTui_buy_handle:',LaiKeTui_buy_handle);
	// alert('LaiKeTui_buy_handle')
	if (me.fastTap) {
		me.fastTap = false;
		me.type = 3;
		me.$refs.lktAuthorizeComp.handleAfterAuth(me, '../../pages/login/login?landing_code=1', function() {
			//正常登录未超时
			// alert('me.haveSkuBean: ' + me.haveSkuBean)
			if (me.haveSkuBean) {
				var product = []
				product.push({
					pid: me.pro_id
				...
				uni.request({
					...
					method: 'POST',
					success: function(res) {
						console.log('LaiKeTui_buy_handle-data',res.data)
						me.clicktimes = [];
						if (res.data.code == 200) {
							...
							console.log('LaiKeTui_buy_handle-me._axios()')
							uni.showToast({
								title: res.data.message,
								duration: 1500,
								icon: 'none'
...
export function LaiKeTui_confirm(me) {
	console.log('me.haveSkuBean:',me.haveSkuBean);
