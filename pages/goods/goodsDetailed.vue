<template>
	<div class="order_ii" :style="{'overflow':overflow,'paddingBottom': (isiPhoneX? '34px':'0')}">
		...
		<div ref='box' id="boxs" class='relative' @touchmove="setHead">
			...
			<!-- #ifdef MP-WEIXIN -->
			<view class="" style="width: 100%;background-color: #fff;" :style="{height: customBar + 'px'}"></view>
			<!-- #endif -->
			<!--头部-->
			<!-- #ifndef MP -->
			<!-- 砍价头部 -->
			...
			<template v-if="loadFlag">
				...
				<ul class='goods_spec' v-if='goods_g'>
				 ...
				 <!-- :style="{'marginBottom': (isiPhoneX? '34px':'0')}" -->
				<div class='goods_foot' v-if='!bargain'>
					<view class="footer-hint" v-if="pro.status == '3'">
						商品已经下架啦~
					</view>
					
					<div class='goods_bottom' :class="{'goods_bottoms':pro.status == '3'}" :style="{'bottom': (isiPhoneX? '34px':'0')}">
		...
		<view 
			...
			v-if='mask_display' 
			:style="{'bottom':(isiPhoneX? '34px':'0'),'paddingBottom': (isiPhoneX? '34px':'0')}"
		>
		</view>
		<view 
			class="p-mask_d"
		...
		>
			...
			<view class='p-mask_querenDiv'>
				<view class='p-mask_quren' :style="{'bottom': (isiPhoneX? '34px':'0')}" @tap="_confirm">确认</view>
			</view>
		</view>
		<!--  遮罩 -->
		...
		<div :class="isiPhoneX?'iphonex':''"></div>
	</div>
</template>
